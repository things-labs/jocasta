...
// network : tcp udp
func NewClient(conn net.Conn, network, target string, timeout time.Duration, auth *proxy.Auth, header []byte) *Client {
	c := &Client{
		conn:    conn,
		network: network,
		timeout: timeout,
...
func (s *Client) SendUDP(data []byte, addr string) (respData []byte, err error) {
...
func (s *Client) handshake(host string) error {
	// the size here is just an estimate
	buf := make([]byte, 0, 6+len(host))

	buf = append(buf, VERSION_V5)
	if len(s.auth.User) > 0 && len(s.auth.User) < 256 && len(s.auth.Password) < 256 {
	...
	if buf[1] == Method_USER_PASS {
		...
		buf = append(buf, uint8(len(s.auth.User)))
		buf = append(buf, s.auth.User...)
		buf = append(buf, uint8(len(s.auth.Password)))
		buf = append(buf, s.auth.Password...)
		s.conn.SetDeadline(time.Now().Add(s.timeout))
		if _, err := s.conn.Write(buf); err != nil {
